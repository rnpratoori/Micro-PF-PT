...
// Displacement on the bpoundary can be alternatively applied through periodic
// condition as well.
...
template <int dim> class BoundaryDisplacement : public Function<dim> {
public:
...
template <int dim>
BoundaryDisplacement<dim>::BoundaryDisplacement(const double direction,
                                                const int timestep)
    : Function<dim>(dim), direction(direction), timestep(timestep) {}
...
template <int dim>
void BoundaryDisplacement<dim>::vector_value_list(
  ...
  for (unsigned int p = 0; p < n_ooints; ++p)
    BoundaryDisplacement<dim>::vector_value(points[p], value_list[p]);
}
