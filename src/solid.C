...
Solid<dim>::Solid(const std::string &input_file)
    : mpi_communicator(MPI_COMM_WORLD),
      ...
      parameters(input_file), time(parameters.end_time, parameters.delta_t),
      ...
      dof_handler(triangulation), dofs_per_cell(fe.dofs_per_cell), u_fe(0),
...
template <int dim> void Solid<dim>::system_setup() {
  ...
  pcout << "   Number of active cells: " << triangulation.n_active_cells()
        ...
        << "   Number of degrees of freedom: " << n_dofs + n_dofs_c << " ("
        << n_dofs << '+' << n_dofs_c << ')' << std::endl;
  ...
  //                                             locally_relevant_dofs_c);
  SparsityTools::distribute_sparsity_pattern(
      dsp_c, dof_handler_c.locally_owned_dofs(), mpi_communicator,
      locally_relevant_dofs_c);
  // Utilities::MPI::all_gather(mpi_communicator, locally_owned_dofs_c);

  mass_matrix.reinit(locally_owned_dofs_c, locally_owned_dofs_c, dsp_c,
                     mpi_communicator);

  solution_c0.reinit(locally_owned_dofs_c, locally_relevant_dofs_c,
                     mpi_communicator);
  solution_c1.reinit(locally_owned_dofs_c, locally_relevant_dofs_c,
                     mpi_communicator);
  solution_c2.reinit(locally_owned_dofs_c, locally_relevant_dofs_c,
                     mpi_communicator);
  solution_c3.reinit(locally_owned_dofs_c, locally_relevant_dofs_c,
                     mpi_communicator);
...
// make_constraints
template <int dim> void Solid<dim>::make_constraints(const int &it_nr) {
  if (it_nr > 1)
    return;
...
template <int dim> void Solid<dim>::assemble_system() {
  ...
  for (; cell != endc; ++cell)
    if (cell->is_locally_owned()) {
      ...
      for (unsigned int q_point = 0; q_point < n_q_points; ++q_point) {
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i) {
          ...
          for (unsigned int j = 0; j < dofs_per_cell; ++j) {
            ...
            cell_matrix(i, j) += symm_grad_Nx[i] *
      ...
      //               ++q_point)
      //                 {
      //                   const Tensor<2, dim> F_inv_tr =
      //                   lqph[q_point].get_F_inv_tr();
      //
      //                     const double J= lqph[q_point].get_det_F();
      //
      //                           double normal_stress;
      //                         if (load_step<12)
      //                              normal_stress= -load_step * J *
      //                              (F_inv_tr*fe_face_values.normal_vector(q_point)).norm();
      //                         else
      //                              normal_stress= -12 * J *
      //                              (F_inv_tr*fe_face_values.normal_vector(q_point)).norm();
      //
      //                         const Tensor<1, dim> traction  = normal_stress*
      ...
      //                       const unsigned int
      //                     component_i =
      //                     fe.system_to_component_index(i).first; cell_rhs(i)
...
template <int dim> void Solid<dim>::assemble_system_c() {
  mass_matrix = 0;
  system_rhs_c1 = 0;
  system_rhs_c2 = 0;
  system_rhs_c3 = 0;
  ...
  typename DoFHandler<dim>::active_cell_iterator cell = dof_handler_c
                                                            .begin_active(),
  ...
  for (; cell != endc; ++cell)
    if (cell->is_locally_owned()) {
      ...
      constraints_c.distribute_local_to_global(cell_rhs_c1, local_dof_indices,
                                               system_rhs_c1);
      constraints_c.distribute_local_to_global(cell_rhs_c2, local_dof_indices,
                                               system_rhs_c2);
      constraints_c.distribute_local_to_global(cell_rhs_c3, local_dof_indices,
                                               system_rhs_c3);
    }
...
template <int dim> void Solid<dim>::update_qph_incremental() {
  ...
  typename DoFHandler<dim>::active_cell_iterator cell =
                                                     dof_handler.begin_active(),
                                                 endc = dof_handler.end();
  typename DoFHandler<dim>::active_cell_iterator cell_c =
      dof_handler_c.begin_active();
...
template <int dim> void Solid<dim>::output_results() const {
    ...
    for (; cell != endc; ++cell)
      if (cell->is_locally_owned()) {
        ...
        norm_of_stress(cell->active_cell_index()) =
            (accumulated_stress / qf_cell.size()).norm();
      } else
  ...
  std::vector<std::vector<Vector<double>>> history_field_stress(
      dim, std::vector<Vector<double>>(dim)),
      local_history_values_at_qpoints_stress(dim,
  ...
  Vector<double> history_field_drivingforce_c1, history_field_drivingforce_c2,
  ...
  FETools::compute_projection_from_quadrature_points_matrix(
      history_fe, qf_cell, qf_cell, qpoint_to_dof_matrix);
        ...
        for (unsigned int j = 0; j < dim; j++) {
          for (unsigned int q = 0; q < qf_cell.size(); ++q) {
            ...
            qpoint_to_dof_matrix.vmult(
                local_history_fe_values_stress[i][j],
            ...
            local_history_values_at_qpoints_drivingforce_c1(q) =
                lqph[q].get_update_c1();
            qpoint_to_dof_matrix.vmult(
                local_history_fe_values_drivingforce_c1,
                local_history_values_at_qpoints_drivingforce_c1);
            ...
            local_history_values_at_qpoints_drivingforce_c2(q) =
                lqph[q].get_update_c2();
            qpoint_to_dof_matrix.vmult(
                local_history_fe_values_drivingforce_c2,
                local_history_values_at_qpoints_drivingforce_c2);
            ...
            local_history_values_at_qpoints_drivingforce_c3(q) =
                lqph[q].get_update_c3();
            qpoint_to_dof_matrix.vmult(
                local_history_fe_values_drivingforce_c3,
                local_history_values_at_qpoints_drivingforce_c3);
  ...
  data_out.add_data_vector(history_dof_handler, history_field_stress[0][0],
                           "sigma_11", data_component_interpretation2);
  data_out.add_data_vector(history_dof_handler, history_field_stress[1][1],
                           "sigma_22", data_component_interpretation2);
  data_out.add_data_vector(history_dof_handler, history_field_stress[2][2],
                           "sigma_33", data_component_interpretation2);
  data_out.add_data_vector(history_dof_handler, history_field_stress[0][1],
                           "sigma_12", data_component_interpretation2);
  data_out.add_data_vector(history_dof_handler, history_field_stress[0][2],
                           "sigma_13", data_component_interpretation2);
  data_out.add_data_vector(history_dof_handler, history_field_stress[1][2],
                           "sigma_23", data_component_interpretation2);

  data_out.add_data_vector(history_dof_handler, history_field_drivingforce_c1,
                           "dc1", data_component_interpretation2);
  data_out.add_data_vector(history_dof_handler, history_field_drivingforce_c2,
                           "dc2", data_component_interpretation2);
  data_out.add_data_vector(history_dof_handler, history_field_drivingforce_c3,
...
template <int dim> void Solid<dim>::output_quad() {
  FEValues<dim> fe_values(fe, qf_cell,
                          update_values | update_gradients |
                              update_quadrature_points | update_JxW_values);
...
template <int dim> void Solid<dim>::output_resultant_stress() {
      ...
      for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell;
        ...
        if (cell->face(face)->at_boundary() &&
          ...
          for (unsigned int q_point = 0; q_point < n_q_points_f; ++q_point) {
            ...
            const double current_element_area =
                J * ((F_inv_tr * fe_face_values.normal_vector(q_point)).norm());

            resultant_force += element_force[2] * fe_face_values.JxW(q_point);
            resultant_pseudo_force +=
                element_pseudo_force[2] * fe_face_values.JxW(q_point);
            current_face_area +=
                current_element_area * fe_face_values.JxW(q_point);
  ...
  resultant_cauchy_stress[time.get_timestep()] =
      -1 * Utilities::MPI::sum(resultant_force, mpi_communicator) /
      Utilities::MPI::sum(current_face_area, mpi_communicator);
  resultant_first_piola_stress[time.get_timestep()] =
      -1 * Utilities::MPI::sum(resultant_force, mpi_communicator) /
      Utilities::MPI::sum(referrence_face_area, mpi_communicator);
  resultant_second_piola_stress[time.get_timestep()] =
      -1 * Utilities::MPI::sum(resultant_pseudo_force, mpi_communicator) /
      Utilities::MPI::sum(referrence_face_area, mpi_communicator);
  resultant_lagrangian_strain[time.get_timestep()] =
      -1 * Utilities::MPI::sum(resultant_E, mpi_communicator) /
      Utilities::MPI::sum(referrence_face_area, mpi_communicator);
...
// solve
template <int dim> unsigned int Solid<dim>::solve() {
...
template <int dim> void Solid<dim>::solve_c() {
  ...
  const double success_tol_c1 =
      (system_rhs_c1.l2_norm() == 0) ? 1.e-6 : 1e-6 * system_rhs_c1.l2_norm();
  const double success_tol_c2 =
      (system_rhs_c2.l2_norm() == 0) ? 1.e-6 : 1e-6 * system_rhs_c2.l2_norm();
  const double success_tol_c3 =
      (system_rhs_c3.l2_norm() == 0) ? 1.e-6 : 1e-6 * system_rhs_c3.l2_norm();
  ...
  solver_c1.solve(mass_matrix, solution_update_c1, system_rhs_c1,
                  preconditioner);
  solver_c2.solve(mass_matrix, solution_update_c2, system_rhs_c2,
                  preconditioner);
  solver_c3.solve(mass_matrix, solution_update_c3, system_rhs_c3,
                  preconditioner);
  ...
  pcout << "Solving for Volume fractions: " << solver_control_c1.last_step()
...
template <int dim> void Solid<dim>::solve_nonlinear_timestep() {
  ...
  for (; newton_iteration < 6000; ++newton_iteration) {
    ...
    if (newton_iteration > 3 &&
        ((system_rhs.l2_norm() <= 1e-4 * initial_rhs_norm) ||
         (system_rhs.l2_norm() <= 1e-09)))
    // if (newton_iteration > 0 && system_rhs.l2_norm() <= 1e-4 *
    // initial_rhs_norm)
    {
